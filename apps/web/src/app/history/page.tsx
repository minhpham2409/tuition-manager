          ...
          <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)', marginBottom: 16 }}>
            ...
            <div className="stat-card yellow">
              ...
              <div style={{ width: '100%', height: 4, background: 'var(--border)', borderRadius: 2, marginTop: 8, overflow: 'hidden' }}>
                <div style={{ width: `${s.total > 0 ? (s.paidCount / s.total) * 100 : 0}%`, height: '100%', background: 'var(--success)', borderRadius: 2, transition: 'width .5s ease' }} />
              </div>
            </div>
          </div>
        )}
          ...
          <div className="card">
            ...
            {data.invoices.length > 0 ? (
              <table className="data-table">
                <thead><tr><th>Học sinh</th><th>Lớp</th><th>Số tiền</th><th>Trạng thái</th><th>Ngày đóng</th><th>Hình thức</th></tr></thead>
                <tbody>
                    ...
                    <tr key={inv.id}>
                      ...
                      <td>
                        {inv.note?.startsWith('Auto:') ? (
                          <span className="badge" style={{ background: 'var(--info-bg)', color: 'var(--info)' }}>Tự động</span>
                        ) : inv.status === 'PAID' ? (
                          <span className="badge" style={{ background: 'var(--warning-bg)', color: 'var(--warning)' }}>Thủ công</span>
                        ) : '—'}
                      </td>
                    </tr>
                  ))}
              ...
              <div className="empty-state"><div className="empty-state-text">Chưa có dữ liệu tháng {month}/{year}</div></div>
            )}
          </div>
        )}
        ...
        {data && view === 'timeline' && (
          <div className="card">
            ...
            {data.timeline.length > 0 ? (
                ...
                {data.timeline.map((evt: any, i: number) => (
                  <div key={i} style={{ position: 'relative', paddingBottom: 20, paddingLeft: 20 }}>
                    <div style={{
                      ...
                      background: evt.type === 'auto' ? 'var(--info)' : 'var(--warning)',
                      border: '3px solid var(--surface)',
                    }} />
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div>
                        <div style={{ fontWeight: 600, color: 'var(--text)', fontSize: '.88rem' }}>{evt.studentName}</div>
                        <div style={{ fontSize: '.78rem', color: 'var(--text-3)', marginTop: 2 }}>
                          {evt.className} · {evt.type === 'auto' ? 'Tự động' : 'Thủ công'}
                        </div>
                      </div>
              ...
              <div className="empty-state"><div className="empty-state-text">Chưa có thanh toán nào</div></div>
            )}
          </div>
        )}
      </main>
