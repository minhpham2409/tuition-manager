      ...
      <div style={{ maxWidth: 600, margin: '0 auto', background: 'var(--surface)', borderRadius: 'var(--radius-xl)', overflow: 'hidden', boxShadow: 'var(--shadow-md)', border: '1px solid var(--border)' }}>
        ...
        <div style={{ padding: '24px 20px' }}>
          <div style={{ marginBottom: 24, padding: 16, background: 'var(--bg-subtle)', borderRadius: 'var(--radius)' }}>
            ...
            <div style={{ fontSize: '.9rem', color: 'var(--text-2)' }}>Lớp: <strong>{data.className}</strong></div>
          </div>
          ...
          {data.attendances.length === 0 ? (
            ...
            <div style={{ border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
              {data.attendances.filter((a: any) => a.taught).map((att: any, i: number) => {
                const date = new Date(att.date);
                const dayName = ['CN', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7'][date.getDay()];
                return (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderBottom: i === data.attendances.length - 1 ? 'none' : '1px solid var(--border-subtle)', background: i % 2 === 0 ? 'var(--surface)' : 'var(--surface-hover)' }}>
                    <div>
                      <div style={{ fontWeight: 500, fontSize: '.88rem', color: 'var(--text)' }}>{dayName}, {date.toLocaleDateString('vi-VN')}</div>
                      {!att.present && att.note && <div style={{ fontSize: '.75rem', color: 'var(--danger)', marginTop: 2 }}>Lý do: {att.note}</div>}
                    </div>
